...
/** Minimal CCIPBridgePlugin interface stub for typing after layer extraction */
interface CCIPBridgePlugin {
  bridgeTokens(params: {
    destChainId: number;
    receiver: string;
    token: string;
    amount: string;
    data: string;
  }): Promise<{ success: boolean; error: string; data: { messageId: string; txHash: string } }>;
  getSupportedLanes(): Array<{ src: number; dest: number }>;
  isChainSupported(chainId: number): Promise<boolean>;
}

/** Minimal chainRegistry stub for typing after layer extraction */
const chainRegistry = {
  get(chainId: number): { ccipChainSelector?: string } | undefined {
    // [LAYER] Implementation moved to @tokenisation/chains
    return undefined;
  },
};
