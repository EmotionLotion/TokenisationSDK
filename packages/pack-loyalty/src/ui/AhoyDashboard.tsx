...
import { LayoutDashboard, Users, Activity, ChevronRight, ArrowUpRight, ArrowDownRight, Database, Plane, Droplets, Map, Radio, Brain, Leaf, Truck, Share2, TrendingUp, Zap, ShieldCheck, Award, Code2 } from 'lucide-react';
// TODO: sdkStore lives in the ui/ app shell — pass as prop or import from shared store package
...
const services: ServiceCard[] = [
  ...
  },
  {
    id: 'ams',
    name: 'AMS/GTS',
    description: 'Movement Data Marketplace',
    icon: Database,
    color: 'text-purple-400',
    bgColor: 'bg-purple-500/10',
    metrics: [
      { label: 'API Developers', value: '234', change: '+15%', positive: true },
      { label: 'Data Queries/Day', value: '89.2K' },
      { label: 'Algorithm NFTs', value: '156' },
    ],
    features: ['Staked API Access', 'Algorithm IP NFTs', 'Royalty Distribution'],
  },
  {
    id: 'trouve',
    name: 'Trouve Labs',
  ...
  },
  {
    id: 'connect',
    name: 'Ahoy Connect',
    description: 'Platform & Marketplace Infrastructure',
    icon: Code2,
    color: 'text-indigo-400',
    bgColor: 'bg-indigo-500/10',
    metrics: [
      { label: 'Connected Accounts', value: '142', change: '+12%', positive: true },
      { label: 'Total Volume', value: '452K', change: '+8%', positive: true },
    ],
    features: ['Hosted KYC', 'Split Payments', 'Platform Fees'],
  },
];
...
// TODO: These demo component imports reference sibling ui/ components that stay in the app shell
// Import from their respective vertical packages once published, or pass as render props
import { CometDemo } from './CometDemo';
import { H2OTODemo } from './H2OTODemo';
import { AMSDemo } from './AMSDemo';
import { IITSDemo } from './IITSDemo';
import { TrouveLabsDemo } from './TrouveLabsDemo';
import { ConnectDemo } from './ConnectDemo';

import { EquityDemo } from './EquityDemo';
import { QuickActionDemo, InlineDemoCard, scenarios } from './QuickActionDemo';
import { AhoyLogo, AhoyTokenSymbol } from './AhoyLogo';

export function AhoyDashboard() {
  ...
  useEffect(() => {
    // Load Ahoy demo data when component mounts
    const loadAhoyData = () => {
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="glass-card rounded-xl p-5 border border-white/10 hover:border-white/20 transition-all duration-300">
              ...
              <p className="text-3xl font-light text-white tracking-tight" style={{ fontFamily: 'Lexend Deca, sans-serif' }}>6</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-1 gap-5">
          ...
          {services.filter(s => s.id === 'equity').map((service) => (
            <div
              ...
              className={`glass-card rounded-xl p-6 cursor-pointer transition-all duration-300 group hover:-translate-y-1 relative overflow-hidden ${selectedService === service.id
                ? 'ring-2 ring-indigo-500 border-transparent bg-white/5'
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
                ...
                <div className="flex items-center gap-8 w-full md:w-auto">
                  ...
                  <div className="ml-auto md:ml-0 bg-white/5 p-2 rounded-full border border-white/10 group-hover:border-indigo-500/50 transition-colors">
                    <ChevronRight className={`w-5 h-5 text-gray-500 transition-transform duration-300 ${selectedService === service.id ? 'rotate-90 text-indigo-500' : 'group-hover:text-white'}`} />
                  </div>
                </div>
              </div>

              {/* Expanded Features */}
              <div className={`grid transition-all duration-300 ease-in-out ${selectedService === service.id ? 'grid-rows-[1fr] opacity-100 mt-6 pt-6 border-t border-white/10' : 'grid-rows-[0fr] opacity-0'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {services.filter(s => s.id !== 'equity').map((service) => (
            <div
              key={service.id}
              onClick={() => setSelectedService(selectedService === service.id ? null : service.id)}
              className={`glass-card rounded-xl p-6 cursor-pointer transition-all duration-300 group hover:-translate-y-1 ${selectedService === service.id
                ? 'ring-2 ring-primary border-transparent bg-white/5'
                : 'border border-white/5 hover:border-white/10 hover:bg-white/5'
                }`}
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <ChevronRight className={`w-5 h-5 text-gray-500 transition-transform duration-300 ${selectedService === service.id ? 'rotate-90 text-primary' : 'group-hover:text-white'
                  }`} />
              </div>
              ...
              {/* Expanded Features */}
              <div className={`grid transition-all duration-300 ease-in-out ${selectedService === service.id ? 'grid-rows-[1fr] opacity-100 mt-4 pt-4 border-t border-white/10' : 'grid-rows-[0fr] opacity-0'
                }`}>
                <div className="overflow-hidden">
                  <p className="text-xs font-bold text-gray-500 uppercase mb-3 tracking-wider">Key Features</p>
                  <div className="flex flex-wrap gap-2">
                    {service.features.map((feature, idx) => (
      ...
      <div className="glass-card rounded-xl border border-white/10 p-6">
        ...
        <div className="relative pt-2">
          ...
          {/* Active Progress Bar Overlay (simplified for visual representation) */}
          <div
