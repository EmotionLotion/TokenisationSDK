...
/** Stub interfaces after ChainlinkFunctionsPlugin moved to @tokenisation/chains */
...
interface ChainlinkFunctionsPlugin {
  calculateSafetyScore(events: Array<{ type: string; count: number }>): Promise<{ success: boolean; error: string; data: SafetyScoreResult }>;
  calculateCarbonOffset(utilityType: string, quantity: number, region: string): Promise<{ success: boolean; error: string; data: CarbonOffsetResult }>;
}
import type { TelematicsReading, TelematicsEventType, DeliveryTracking } from '../plugins/TelematicsOracle.js';
import { NetworkError, ValidationError, AuthenticationError, ErrorCode } from '@tokenisation/core';
...
export class CometDataAdapter {
  ...
  async getDriver(driverId: string): Promise<CometDriver | null> {
    try {
      ...
      return await response.json() as CometDriver;
    } catch (error) {
  ...
  async getDriverTelematicsEvents(
    ...
    try {
      ...
      return await response.json() as CometTelematicsEvent[];
    } catch (error) {
  ...
  async getDelivery(deliveryId: string): Promise<CometDelivery | null> {
    try {
      ...
      return await response.json() as CometDelivery;
    } catch (error) {
