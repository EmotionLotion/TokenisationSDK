 ...
 *
 * Moved from @tokenisation/core to @tokenisation/pack-loyalty.
 */

import { ValidationError } from '@tokenisation/core';

// ============================================================================
// ADAPTER INTERFACE TYPES (from core adapter specification)
// ============================================================================

type TokenStandard = 'ERC-3643' | 'ERC-1400' | 'ERC-20' | 'ERC-721' | 'ERC-1155';
...
interface AssetSchema {
  type: 'object';
  required: string[];
  properties: Record<string, { type: string; description?: string; format?: string; enum?: string[]; minimum?: number; maximum?: number; items?: { type: string; enum?: string[] } }>;
}
...
interface IssueContext extends AssetContext {
  tokenId: string;
  recipientId: string;
...
interface TransferContext extends AssetContext {
  tokenId: string;
  transferId: string;
  fromAddress: string;
  toAddress: string;
...
interface RedeemContext extends AssetContext {
  tokenId: string;
  holderId: string;
...
interface PayoutContext extends AssetContext {
  tokenId: string;
...
interface IAssetAdapter<TAssetInput = Record<string, unknown>, TAssetMetadata = Record<string, unknown>> {
