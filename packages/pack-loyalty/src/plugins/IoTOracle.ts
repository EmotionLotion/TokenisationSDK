...
/** Stub interface after ChainlinkFunctionsPlugin moved to @tokenisation/chains */
interface ChainlinkFunctionsPlugin {
  calculateSafetyScore(events: Array<{ type: string; count: number }>): Promise<{ success: boolean; error: string; data: { score: number; calculatedAt: string } }>;
  calculateCarbonOffset(utilityType: string, quantity: number, region: string): Promise<{ success: boolean; error: string; data: { offsetKg: number; methodology: string; calculatedAt: string } }>;
}
