...
import {
    Building2, Plane, Car, Hotel, Music, Home, Search, Bell, ChevronDown, Code2
} from 'lucide-react';
import { SdkInsightPanel } from './SdkInsightPanel';
...
const SDK_NAV = [
    { id: 'home', icon: Home, label: 'SDK Home', color: 'text-[#F8B032]' },
    { id: 'real-estate', icon: Building2, label: 'Real Estate', color: 'text-emerald-400' },
    { id: 'airline', icon: Plane, label: 'Airline Tickets', color: 'text-sky-400' },
    { id: 'car-rental', icon: Car, label: 'Car Rental', color: 'text-orange-400' },
    { id: 'hotel', icon: Hotel, label: 'Hotel Reservation', color: 'text-purple-400' },
    { id: 'concert', icon: Music, label: 'Concert Tickets', color: 'text-pink-400' },
];

export function SDKLayout({ children, activeTab, onTabChange }: SDKLayoutProps) {
    const renderNavButton = (item: typeof SDK_NAV[0]) => {
        ...
        return (
            ...
            >
                <Icon className={`w-5 h-5 transition-colors ${
                    isActive ? item.color : 'group-hover:text-[#F8B032]/70'
                }`} />
        ...
        <div className="flex h-screen bg-background text-white overflow-hidden selection:bg-primary/30">
            ...
            <aside className="relative z-20 w-64 glass border-r border-white/5 flex flex-col backdrop-blur-xl">
                ...
                <div className="h-20 flex items-center px-5 border-b border-[#F8B032]/10 bg-gradient-to-r from-[#0F172A] to-transparent">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-[#F8B032]">SDK Use Cases</p>
                        </div>
                    </div>
                </div>
                ...
                <nav className="flex-1 py-4 px-3 overflow-y-auto">
                    <p className="text-[10px] uppercase tracking-[0.2em] text-gray-600 font-medium mb-3 px-3">SDK Verticals</p>
                    <div className="space-y-1">
                        {SDK_NAV.map(renderNavButton)}
                    </div>
                </nav>
                ...
                <div className="px-4 py-3 border-t border-white/5">
                    <button className="w-full flex items-center justify-between px-3 py-2 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-gray-300">SDK Demo</span>
                        </div>
                ...
                <header className="h-16 glass border-b border-white/5 flex items-center justify-between px-8 backdrop-blur-md sticky top-0 z-30">
                    ...
                    <div className="relative w-96 group">
                        ...
                        <input
                            ...
                            placeholder="Search SDK use cases..."
                        />
                    </div>
